  ...
  _createClass(PathAutocomplete, [{
    ...
    value: function findMatches() {
      ...
      .filter(function (child) {
        return _this.caseSensitive ? child.startsWith(prefix) : child.toLowerCase().startsWith(prefix.toLowerCase());
      })
      // Map to Path instances
      .map(function (name) {
        var childPath = directory.getRelativePath() + name;
        try {
          if (_fs2.default.statSync(_path2.default.join(_this.cwd.absolutePath, childPath)).isDirectory()) {
